...
const PROJECTS: Project[] = [
  {
    ...
    tags: [
      { label: 'Python', color: 'bg-amber-500/10 text-amber-700 dark:text-amber-400 border-amber-500/20' },
      { label: 'YOLO', color: 'bg-rose-500/10 text-rose-700 dark:text-rose-400 border-rose-500/20' },
      { label: 'ONNX', color: 'bg-indigo-500/10 text-indigo-700 dark:text-indigo-400 border-indigo-500/20' },
      { label: 'Raspberry Pi', color: 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-emerald-500/20' },
    ],
    icon: (
  ...
  },
  {
    ...
    tags: [
      { label: 'JavaScript', color: 'bg-amber-500/10 text-amber-700 dark:text-amber-400 border-amber-500/20' },
      { label: 'Manifest V3', color: 'bg-cyan-500/10 text-cyan-700 dark:text-cyan-400 border-cyan-500/20' },
      { label: 'Chrome APIs', color: 'bg-indigo-500/10 text-indigo-700 dark:text-indigo-400 border-indigo-500/20' },
    ],
    icon: (
      <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
  ...
  },
  {
    ...
    tags: [
      { label: 'Azure Functions', color: 'bg-indigo-500/10 text-indigo-700 dark:text-indigo-400 border-indigo-500/20' },
      { label: 'PostgreSQL', color: 'bg-cyan-500/10 text-cyan-700 dark:text-cyan-400 border-cyan-500/20' },
      { label: 'Serverless', color: 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-emerald-500/20' },
    ],
    icon: (
      <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
function ProjectCard({ project, index }: { project: Project; index: number }) {
  ...
  return (
    <div
      ref={ref}
      className={`group flex flex-col rounded-2xl border border-slate-200 bg-white p-6 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg dark:border-slate-800 dark:bg-slate-900/50 dark:hover:border-slate-700 ${
        inView ? 'animate-fade-in-up' : 'opacity-0'
      }`}
      style={{ animationDelay: `${index * 150}ms` }}
    >
      {/* Icon */}
      <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-slate-100 text-slate-600 transition-colors group-hover:bg-indigo-50 group-hover:text-indigo-600 dark:bg-slate-800 dark:text-slate-400 dark:group-hover:bg-indigo-500/10 dark:group-hover:text-indigo-400">
        {project.icon}
      </div>
